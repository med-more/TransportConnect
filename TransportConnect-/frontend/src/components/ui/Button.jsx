...
const Button = forwardRef(
  ({ children, className, variant = "primary", size = "medium", loading = false, disabled = false, ...props }, ref) => {
    const baseClasses =
      "inline-flex items-center justify-center font-medium rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-background disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98] disabled:active:scale-100"
    ...
    const sizes = {
      small: "h-8 px-2.5 sm:px-3 text-xs sm:text-sm",
      medium: "h-9 sm:h-10 px-3 sm:px-4 text-xs sm:text-sm",
      large: "h-10 sm:h-11 px-5 sm:px-6 text-sm sm:text-base",
